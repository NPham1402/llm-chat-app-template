  ...
  async function handleSendMessage(event) {
    ...
    try {
      ...
      if (error?.name !== 'AbortError') {
        ...
        messages = messages.map((m, i) =>
          i === assistantIndex
            ? { ...m, content: 'Xin lỗi, đã có lỗi xảy ra khi xử lý yêu cầu của bạn.' }
            : m
        );
      }
    } finally {
...
<div class="min-h-screen bg-gray-50 flex flex-col">
  <!-- Header -->
  <header class="bg-white border-b border-gray-200 py-4 px-4 text-center">
    <h1 class="text-2xl font-bold text-green-600">Nông Lâm Viên ChatBox</h1>
  </header>
  ...
  <footer class="text-center py-4 text-sm text-gray-500">
    <p>Nông lâm viên chatbox &copy; 2025</p>
  </footer>
</div>
